...
_x = range(len(a))
plt.yticks(_x,a, fontproperties=myFont, rotation=0)

plt.barh(_x,b,height=0.3,color='orange')

plt.xlabel("票房(单位:亿元)",fontproperties=myFont, loc='right')
plt.ylabel("电影名称",fontproperties=myFont, loc='top')

# 在每个条形图上添加数值标签
for i, value in enumerate(b):
    plt.text(value + 0.1, i, '%.2f' % value, ha='left', va='center', fontproperties=myFont)
